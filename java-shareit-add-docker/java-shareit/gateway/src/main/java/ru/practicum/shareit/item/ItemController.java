...
public class ItemController {
    ...
    @PostMapping
    public ResponseEntity<Object> saveItem(@RequestHeader("X-Sharer-User-Id") long userId, @RequestBody @Validated(Create.class) ItemDtoGate itemDto) {
        log.info("Поступил запрос в API Gateway - saveItem");
        return itemClient.saveItem(userId, itemDto);
    }

    @PatchMapping("/{itemId}")
    public ResponseEntity<Object> updateItem(@RequestHeader("X-Sharer-User-Id") long userId, @RequestBody @Validated(Update.class) ItemDtoGate itemDto, @PathVariable Long itemId) {
    ...
    public ResponseEntity<Object> addComment(@RequestHeader("X-Sharer-User-Id") long userId, @RequestBody @Validated CommentDtoGate commentDtoIn, @PathVariable Long itemId) {
